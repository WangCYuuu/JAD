...
与 joint_attention_fusion.py 的接口保持一致：
- GradCAMExtractor / ViTGradCAMExtractor：注意力提取
- JointAttentionFusion：注意力融合
- JointAttentionLoss：注意力一致性损失
- create_joint_attention_system：系统构建函数

区别：JointAttentionFusion 的跨模型融合从“动态权重”改为“固定权重的简单加权平均”。
"""
...
class JointAttentionFusion(nn.Module):
    ...
    def forward(
        ...
        # Step 2: 跨模型简单加权平均融合（固定权重）
        w_cnn = torch.full((B,), self.w_cnn, device=device, dtype=A_cnn.dtype)
        w_vit = torch.full((B,), self.w_vit, device=device, dtype=A_vit.dtype)
...
def create_joint_attention_system(
    ...
    # 创建融合模块
    fusion_module = JointAttentionFusion(
        cnn_layers,
        vit_blocks,
        target_size,
        w_cnn=fusion_weights[0],
        w_vit=fusion_weights[1],
    )
